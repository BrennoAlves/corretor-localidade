...
SCRIPT_PEGAR_CIDADES = 'pegar_cidades.py'
VENV_CAMINHO = "venv/Scripts/python.exe"
...
def carregar():
    ...
    cidades_canonicas = json.load(open(CIDADES_CANONICAS_JSON, "r", encoding="utf-8"))
...
def verificar_cidades_canonicas(arquivo):

    if not os.path.exists(arquivo):
        print(f"A lista de cidades canônicas não foi encontrada. Consultando a API do IBGE")
        try:
            result = subprocess.run([VENV_CAMINHO, 'Src/pegar_cidades.py'])
...
def normalizar_cidades(df_imoveis, cidades_canonicas, modelo):
    ...
    try:
        embeddings_cidades_canonicas = pickle.load(
            open(CIDADES_CANONICAS_PKL, 'rb'))
    except:
