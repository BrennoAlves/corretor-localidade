...
def carregar_dados(tipos_csv, imoveis_json, modalidades, status):
    ...
    return df_imoveis, cidades_canonicas, modelo, bairros_canonicos
...
def normalizar_bairros(df_imoveis, bairros_canonicos, modelo, embeddings_path): 
    ...
    try:
        ...
        for bairro_normalizado, _ in bairros_canonicos.items():
            embeddings_bairros_canonicos[bairro_normalizado] = modelo.encode(
                bairro_normalizado, convert_to_tensor=True, device="cuda"
            )
...
if __name__ == "__main__":
    ...
    df_imoveis, cidades_canonicas, modelo, bairros_canonicos = carregar_dados(TIPOS_CSV, IMOVEIS_JSON, MODALIDADES, STATUS)
