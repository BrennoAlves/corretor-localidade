...
CIDADES_CANONICAS_JSON = os.path.join(os.path.dirname(
    __file__), "..", "data", "cidades_canonicas.json")
BAIRROS_CANONICOS_JSON = os.path.join(os.path.dirname(
    __file__), "..", "data", "bairros_canonicos.json")
...
def obter_bairros_por_cidade(nome_cidade):
    ...
    nome_cidade_url = unidecode.unidecode(
        nome_cidade).lower().replace(' ', '-')
...
def main(df_imoveis):
    ...
    for cidade in cidades_no_df:
        if pd.isnull(cidade):
            continue
        nomes_bairros = obter_bairros_por_cidade(cidade)
        for bairro in nomes_bairros:
            bairros_canonicos[bairro] = cidade
